...
var execSync = require('scribe-test-harness/node_modules/execsync');
var mkdirp = require('scribe-test-harness/node_modules/mkdirp');
  ...
  function setupTest(options, done) {
    require([
      ...
      var scribe = window.scribe,
        opts,
        defaultSanitizerConfig = function () {
          return {
      ...
      function enableOnElement (el, opts) {
        ...
        if (scribe.getSanitizerByName) {

          var sanitizer = scribe.getSanitizerByName('defaultSanitizer');

          if (sanitizer) {
            var config = defaultSanitizerConfig().tags;
            ...
            sanitizer.setConfig({tags : config});
          }
      ...
      if (!scribe) {
        window.scribe = scribe = new Scribe(null, opts);

        // scribe.use(scribePluginToolbar(window.document.querySelector('.rte-toolbar')));
        ...
        if (opts.sanitize) {
          scribe.use(scribePluginSanitizer((defaultSanitizerConfig())));
          delete opts.sanitize;
        }
...
beforeEach(function () {
  ...
  exports.scribeNode2 = exports.driver.findElement(webdriver.By.className('scribe2'));
});
